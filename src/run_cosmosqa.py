...
class CosmosqaDataset(Dataset):
    ...
    DATA_TYPE_TO_FILENAME = {
        "train":"cosmosqa-data/train.jsonl",
        "train_label":"cosmosqa-data/train-labels.lst",
        "dev":"cosmosqa-data/valid.jsonl",
        "dev_label":"cosmosqa-data/valid-labels.lst",
        "test":"cosmosqa-data/test.jsonl"}
    ...
    def __init__(self,model_class, data_type, data_dir, tokenizer, do_lower_case, max_seq_length, **kwargs):
        ...
        for i, line in enumerate(raw_examples):
            ...
            answers = np.array([line["answer{}".format(e)] for e in range(len(self.LABELS))])
    ...
    def __getitem__(self, item):
        ...
        context_tokens = self.tokenizer.tokenize(example["context"]+example["question"])
        answers_tokens = [self.tokenizer.tokenize(e_) for e_ in example["answers"]]
        ...
        for choice_idx, answer_tokens in enumerate(answers_tokens):
            if self.model_class == "bert":
                _truncate_seq_pair(context_tokens, answer_tokens, self.max_seq_length - 3)
            else:
                _truncate_seq_pair(context_tokens, answer_tokens, self.max_seq_length - 4)
    ...
    for line_ in tqdm(predictions, ncols=100, desc="check questions where model failed to answer"):
        ...
        for i in range(len(ls)):
            if ls[i]["qid"] == qid_:
                s = ls[i]
                if id_ != s["label"]:
                    data.append({"qid":qid_, "context":s["context"], "question":s["question"],"answers":s["answers"], "label":s["label"], "id":id_})
    torch.save(data, fp)
